...
class block_newresources extends block_base {
    ...
    function cron() {
		...
		//Check if today is the correct day of the week to send notifications
		$day = date('N');
		$freqnotifyday = get_config('newresources', 'freqnotifyday');
		if ($freqnotifyday != 0 && $freqnotifyday != $day) {
			mtrace('Not notification day');
			return;
		}

		//Check if notification cron already run today
		$lastrun = get_config('newresources','lastrun');
		$now = time();
		if ($now < $lastrun + 23*60*60) {
			mtrace('Cron already run today');
			return;
		} else { //If not, update the config to keep the timestamp of last cron run
			set_config('lastrun',$now,'newresources');
		}

		//Check if is time to send notifications
		$hourstimenotify = get_config('newresources', 'hourstimenotify');
		$minutestimenotify = get_config('newresources', 'minutestimenotify');
		if ($now<strtotime('02:00')+$hourstimenotify*60*60+$minutestimenotify*60) { //everything is GMT, but no problem...
			mtrace('It is not time to send notification yet');
			return;
		}


		// PROCESSO ================
		// 1) Obter lista dos recursos incluidos no ultimo dia ou ultima semana			// 2) Ordenar modulos novos por COURSEID
		...
		$mods = $DB->get_records_sql('SELECT cm.id, course.id AS courseid, course.shortname AS shortname, course.fullname AS coursename, cm.module AS moduleid, cm.instance, cm.section, cm.added, cm.visible, mods.name AS modulename 
					...
					'.$query.'
					ORDER BY courseid ASC'); //IMPORTANT: order by COURSEID
		if ($mods) {
